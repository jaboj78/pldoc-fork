...
CREATE OR REPLACE PROCEDURE prc_after_as(pa_last_date IN date, pa_epsfile IN varchar2, pa_publ_id IN varchar2, pa_logging_level IN varchar2 := 'INFO') 
AS
/**
<P>Update PRC_AFTER_AS[publ_id=pa_publ_id AND epsfile=pa_epsfile].GRAPHIC_LAST_MODIFIED with modification date from file system.</P>
