...
#use the standalone PLDoc jar and Oracle JDBC as normal
...
then
	...
	then
	  JDBC_HOME="${ORACLE_HOME}"/jdbc/lib 
	elif [ -d "${ORACLE_HOME}" ]
	then
	  JDBC_HOME="${ORACLE_HOME}" 
	fi 
	if [ "${JDBC_HOME}" =  "" ]
	then
		...
		#It seems that because the PLDoc class is loaded from endorsed dirs, then the JDBC classes loaded in it must also be loaded from endorsed dirs   
 		export ANT_OPTS="-server -Djava.endorsed.dirs=${PLDOC_HOME}/target:${JDBC_HOME} ${INITIAL_ANT_OPTS}"  
